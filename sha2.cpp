...
const std::uint64_t SHA512::sha512_k[80] = 
...
void SHA224::update(const char *message, unsigned int len)
{
	...
	auto shifted_message = message + rem_len;
	transform(m_block, 1);
...
void SHA256::update(const char *message, unsigned int len)
{
	unsigned int block_nb;
	unsigned int new_len, rem_len, tmp_len;
	tmp_len = SHA224_256_BLOCK_SIZE - m_len;
...
void SHA384::update(const char *message, unsigned int len)
{
	unsigned int block_nb;
	unsigned int new_len, rem_len, tmp_len;
	//unsigned char *shifted_message;
...
void SHA512::update(const char *message, unsigned int len)
{
	unsigned int block_nb;
	unsigned int new_len, rem_len, tmp_len;
	tmp_len = SHA384_512_BLOCK_SIZE - m_len;
